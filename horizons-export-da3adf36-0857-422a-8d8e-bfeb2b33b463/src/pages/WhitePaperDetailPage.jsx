...
const WhitePaperDetailPage = ({ showToast }) => {
  const { whitePaperSlug } = useParams();
  ...
  useEffect(() => {
    const fetchDocument = async () => {
      ...
      try {
        ...
        if (!data) throw new Error("White Paper not found.");
        setDocument(data);
      } catch (err) {
        setError(err.message);
        showToast(`Error fetching white paper: ${err.message}`);
      } finally {
        setLoading(false);
      }
    };
  ...
  }, [whitePaperSlug, showToast]);

  if (loading) {
    return (
      <div className="container mx-auto px-4 py-12 text-center">
        <div className="animate-spin rounded-full h-12 w-12 border-b-2 border-primary mx-auto"></div>
        <p className="mt-4 text-lg text-muted-foreground">Loading White Paper...</p>
      </div>
    );
  }
  ...
  if (error || !document) {
    return (
      <div className="container mx-auto px-4 py-12 text-center">
        <Meta title="White Paper Not Found" />
        <p className="text-lg text-destructive">
          {error ? `Failed to load document: ${error}` : "White Paper not found."}
        </p>
        <Button asChild variant="outline" className="mt-6">
          <Link to="/white-papers">
            <ArrowLeft className="mr-2 h-4 w-4" /> Back to White Papers
          </Link>
        </Button>
      </div>
    );
  ...
  const pageTitle = `${document.title} - SELFIX White Paper`;
  const pageDescription = document.abstract || `Explore the SELFIX White Paper: ${document.title}.`;
    ...
    <>
      ...
      <div className="container mx-auto px-4 sm:px-6 lg:px-8 py-16">
        ...
        >
          ...
          <p className="text-lg text-muted-foreground mb-10">{document.publication_code}</p>
        </motion.div>
                ...
                {document.pdf_url ? (
                  <div className="aspect-[8.5/11] w-full bg-slate-900/50 rounded-md overflow-hidden border border-border">
                    <iframe
                      ...
                      allowFullScreen
                    >
                      Your browser does not support embedded PDFs. You can <a href={document.pdf_url} target="_blank" rel="noopener noreferrer" className="text-primary hover:underline">download the PDF directly</a>.
                    </iframe>
                  </div>
                ) : (
            ...
            <Card className="bg-card/50 backdrop-blur-sm border-border shadow-lg sticky top-24 cyber-border-soft">
              ...
              <CardContent className="space-y-5">
                <div>
                  ...
                  <p className="text-foreground">{document.author || 'DAOCORE/SELFIX'}</p>
                </div>
                <div>
                  ...
                  <p className="text-foreground">{new Date(document.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}</p>
                </div>
                <div>
                  ...
                  <p className="text-foreground break-all text-xs">{document.dao_signature || 'Not Available'}</p>
                </div>
                 <div>
                  <h3 className="text-sm font-medium text-muted-foreground flex items-center"><FileText className="mr-2 h-4 w-4" />License</h3>
                  <p className="text-foreground">{document.license || 'SmartLicense-X™'}</p>
                </div>
                {document.abstract && (
                  <div>
                    <h3 className="text-sm font-medium text-muted-foreground">Abstract</h3>
                    <p className="text-foreground text-sm leading-relaxed">{document.abstract}</p>
                  </div>
                )}
                ...
                <div className="pt-4 border-t border-border">
                  ...
                  {document.ipfs_hash && (
                    <Button asChild variant="outline" size="lg" className="w-full mt-3">
                      <a href={`https://ipfs.io/ipfs/${document.ipfs_hash}`} target="_blank" rel="noopener noreferrer">
