...
import { coreTechModules } from '@/data/landing/technology.jsx';
import { CheckCircle } from 'lucide-react';
import Meta from '@/components/Meta';

const TechnologyCard = ({ tech }) => {
    return (
        <Card className="h-full bg-background/50 backdrop-blur-sm cyber-border-soft hover:border-primary/80 transition-all duration-300 transform hover:-translate-y-2 flex flex-col">
            <CardHeader className="text-center">
                <div className="mx-auto bg-primary/10 p-4 rounded-full w-fit mb-4">
                    {tech.icon}
                </div>
                <CardTitle className="text-primary font-semibold text-2xl">{tech.title}</CardTitle>
...
const TechnologyPage = () => {
    return (
        <>
        <Meta
            title="Investor Deep Dive: Core Technology | SELFIX"
            description="Explore the proprietary technologies that power SELFIX, from our AI Healing Engine to the Entropy Framework. This is our defensible edge in self-healing cybersecurity."
        />
        <section id="tech" className="py-20 sm:py-28 bg-background">
          <div className="container mx-auto px-4 sm:px-6 lg:px-8">
            <div className="text-center mb-16">
              <h1 className="text-4xl sm:text-5xl font-bold tracking-tight page-title-glow">Investor Deep Dive: Core Technology</h1>
